...
Endpoint Agent 注册后消费所属 Host 的 `api.events.host` 流。共享的所有权逻辑根据已发布的工作区确定帧归属，通过 `hub/agent/host-event` 转发未修改的帧，并在断连时 abort 且等待流结束。端点级帧使用空工作区 ID；无法唯一确定项目归属的项目帧会停止桥接，而不会任意选择工作区。
