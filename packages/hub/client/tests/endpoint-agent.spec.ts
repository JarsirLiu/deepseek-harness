...
describe('HubEndpointAgent', () => {
  it('registers workspaces and publishes a normalized registration request', async () => {
    ...
    await expect(agent.connect([workspace])).resolves.toMatchObject({ endpointId: 'remote:agent' })
  ...
  it('clears the socket and registration state when registration fails', async () => {
    ...
    await expect(agent.connect([workspace])).rejects.toThrow('registration rejected')
    expect(agent.registrationResult).toBeNull()
    ...
    FakeWebSocket.behavior = 'invalid-registration'
    await expect(invalidAgent.connect([workspace])).rejects.toThrow('invalid Endpoint Agent registration response')
  ...
  it('rejects duplicate connections and unknown workspace host events', async () => {
    ...
    await expect(agent.connect([workspace])).rejects.toThrow('already connected')
    expect(() => { agent.publishHostEvent('workspace-missing', { type: 'host/session-status' } as never) })
      .toThrow('workspace unavailable')
  })

  it('publishes workspace replacements and unchanged Host frames', async () => {
    const agent = createAgent()
    await agent.connect([workspace])
    await agent.publishWorkspaces([{ ...workspace, id: 'workspace-b' }])
    agent.publishHostEvent('workspace-b', { type: 'host/session-status', sessionId: 's1', running: true } as never)

    const socket = FakeWebSocket.instances.at(-1)!
    const messages = socket.sent.map(message => JSON.parse(message) as { method: string; params?: unknown })
    expect(messages.map(message => message.method)).toEqual([
      'hub/agent/register', 'hub/agent/publish-workspaces', 'hub/agent/host-event',
    ])
