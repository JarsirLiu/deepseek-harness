...
describe('RemoteSessionProvider', () => {
  ...
  it('connects once, performs handshake, and subscribes selected workspaces', async () => {
    ...
    await provider.subscribeWorkspaces([{ endpointId: 'remote:test', workspaceId: 'workspace-a' }])
    ...
    expect(messages[1]?.params).toEqual({ workspaces: [{ endpointId: 'remote:test', workspaceId: 'workspace-a' }] })
  })
  ...
  it('dispatches session, wildcard, status, and host notifications and disposes listeners', async () => {
    ...
    const status = vi.fn()
    const host = vi.fn()
    const disposeSession = provider.subscribe('workspace-a', 'session-a' as never, session)
    ...
    socket.emit('message', JSON.stringify({ jsonrpc: '2.0', method: 'hub/event', params: { workspaceId: 'workspace-a', sessionId: 'session-a' } }))
    socket.emit('message', JSON.stringify({ jsonrpc: '2.0', method: 'hub/status', params: { status: 'running' } }))
    socket.emit('message', JSON.stringify({ jsonrpc: '2.0', method: 'hub/host', params: { endpointId: 'remote:test', workspaceId: 'workspace-a', frame: { type: 'host/session-status' } } }))
    expect(session).toHaveBeenCalledTimes(1)
    expect(wildcard).toHaveBeenCalledTimes(1)
    expect(status).toHaveBeenCalledTimes(1)
    expect(host).toHaveBeenCalledTimes(1)
    socket.emit('message', JSON.stringify({ jsonrpc: '2.0', method: 'hub/event', params: { workspaceId: 'workspace-a', sessionId: 'unsubscribed' } }))
  ...
  it('removes the remote session subscription after the last listener is disposed', async () => {
    ...
    const first = vi.fn()
    const second = vi.fn()
    const disposeFirst = provider.subscribe('workspace-a', 's1' as never, first)
    const disposeSecond = provider.subscribe('workspace-a', 's1' as never, second)
    disposeFirst()
    disposeSecond()
    disposeSecond()
    await new Promise(resolve => setTimeout(resolve, 0))
